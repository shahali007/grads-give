...
const HomePageBanner = () => {
    ...
    const [selectedBanner, setSelectedBanner] = useState({
        ...
        bannerHeading: "Fight against student Hunger",
        bannerDescription:
            "Congress cut $11B child nutrition waiver program, ending subsidized lunches for millions of students. Help feed millions hungry children that rely on meal programs for daily nutrition.",
    });
        ...
        <div className="min-h-220">
            ...
            <div className="flex">
                {bannerTypes.map((banner, index) => (
                    <div
                        className={`w-1/3 h-46 p-4 lg:p-10 flex justify-center items-center flex-col-reverse sm:block cursor-pointer ${banner.selected ? "bg-primaryColor" : "bg-secondaryColor"} border-r-1`}
                        key={index}
                        onClick={() => handleBannerSelection(banner)}
                    >
                        <p className="font-bold text-xl sm:text-2xl text-center sm:text-left">{banner.name}</p>
